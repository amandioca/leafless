...
public class AssociarGrupoUsuario extends javax.swing.JFrame {
    ...
    private void cancelarAcaoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cancelarAcaoActionPerformed
        ...
        int resposta = JOptionPane.showOptionDialog(null, "Tem certeza que deseja cancelar a ação?", "Cancelar Associação", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
        if (resposta == JOptionPane.YES_OPTION) {
            TelasUtil.voltar();
        }
